...
log = logging.getLogger("exstrorezov")
...
def _build_backup_text():
    ...
    if ch_rows:
        progress_parts = [f"{r[0]}:{r[1]}" for r in ch_rows[:50]]
...
def _parse_backup_lines(text):
    ...
    if not text or not text.startswith("📦 Бэкап настроек Exstrorezov"):
        return False
    for line in text.split("\n"):
...
async def backup_settings_to_telegram():
    """Сохраняет настройки обоим владельцам в ЛС (без канала вывода)"""
    try:
        backup_text = _build_backup_text()
        
        # Шлём обоим владельцам (каждый видит бэкап в своей личке с ботом)
