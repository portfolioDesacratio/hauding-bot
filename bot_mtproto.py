...
FALLBACK_OUTPUT_CHAT = -1003997635438  # новый канал вывода (постоянный, бот там админ)
...
async def _resume_file_streaming():
    ...
    try:
        ...
        # Проверка: не завершён ли уже файл? (bytes_offset >= file_size или words_sent близко к концу)
...
async def cmd_setfp(event):
    ...
    try:
        ...
        if not (fp_chat.lstrip("-").isdigit() and fp_msg.isdigit()
            ...
            return
        fp_size_int = int(fp_size)
        fp_bytes_int = int(fp_bytes)
        fp_words_int = int(fp_words)
        # Защита: если файл уже завершён (bytes_offset >= file_size), не возобновляем
        if fp_size_int > 0 and fp_bytes_int >= fp_size_int:
            await safe_send(event.chat_id,
                f"⚠️ bytes_offset ({fp_bytes_int}) >= file_size ({fp_size_int}) — "
                f"файл уже полностью отправлен. Прогресс не сохранён.")
            return
        _save_file_progress(
            int(fp_chat), int(fp_msg), fp_fn,
            0, fp_words_int, 0, 0, fp_size_int,
            checkpoint_msg_id=None,
            bytes_offset=fp_bytes_int
        )
        await safe_send(event.chat_id,
            f"✅ Прогресс сохранён: {fp_fn}, words_sent={fp_words_int}, bytes_offset={fp_bytes_int}\n"
